...
package org.eclipse.rdf4j.sail.leveldb.benchmark;
...
import org.eclipse.rdf4j.sail.leveldb.NativeStore;
...
public class TransactionsPerSecondBenchmark {
	...
	public void largerTransaction() {
		...
		for (int k = 0; k < 10000; k++) {
			IRI resource = connection.getValueFactory().createIRI("resource:" + (i / 10));
			IRI property = connection.getValueFactory().createIRI("property:" + (i / 10));
			connection.add(resource, property, connection.getValueFactory().createLiteral(i++ + "_" + k));
			//connection.add(RDFS.RESOURCE, RDFS.LABEL, connection.getValueFactory().createLiteral(i++ + "_" + k));
		}
		connection.commit();
	}
