 ...
 * <p>
 * The WAVL tree deletion is described in the 2015 paper "Rank Balanced Trees" The related RAVL tree is described in the 2016 paper
 * "Deletion Without Rebalancing in Binary Search Trees" both are available at: http://sidsen.azurewebsites.net//
 *
 * @param <K> the type of keys maintained by this map
 * @param <V> the type of mapped values
 * @author David McManamon
 */
public class WAVLTreeMap<K, V> extends AbstractMap<K, V> {
    ...
    public WAVLTreeMap(boolean deleteWAVL, Comparator<K> comp) {
     ...
     * {@code k} to a value {@code v} such that {@code key} compares equal to {@code k} according to the map's ordering, then this method
     * returns {@code v}; otherwise it returns {@code null}. (There can be at most one such mapping.)
     *
     * <p>A return value of {@code null} does not <em>necessarily</em>
     * indicate that the map contains no mapping for the key; it's also possible that the map explicitly maps the key to {@code null}. The
     ...
     * @throws NullPointerException if the specified key is null and this map uses natural ordering, or its comparator does not permit null
     *                              keys
     */
    ...
    /**
     * Version of getEntry using comparator. Split off from getEntry for performance. (This is not worth doing for most methods, that are
     * less dependent on comparator performance, but is worthwhile here.)
     */
     ...
     * @return the previous value associated with {@code key}, or {@code null} if there was no mapping for {@code key}. (A {@code null}
     * return can also indicate that the map previously associated {@code null} with {@code key}.)
     * @throws ClassCastException   if the specified key cannot be compared with the keys currently in the map
     * @throws NullPointerException if the specified key is null and this map uses natural ordering, or its comparator does not permit null
     *                              keys
     */
    ...
    /**
     * - If the path of incremented ranks reaches the root of the tree stop. - If the path of incremented ranks reaches a node whose
     * parent's rank previously differed by two and after incrementing now differ by one stop. - If the procedure increases the rank of a
     * node x, so that it becomes equal to the rank of the parent y of x, but the other child of y has a rank that is smaller by two (so
     * that the rank of y cannot be increased) then again the rebalancing procedure stops after performing rotations necessary. In other
     * words: After insertion rank difference is 1,2 or 3 - check these three cases stopping after any rotations, reaching the root or when
     * rank difference was 2 before the insertion.
     */
    ...
    public Iterator<Map.Entry<K, V>> iterator(Object key) {
    	return new EntryIterator(getEntry(key));
	}
     ...
     *
     * <p>
     * The set's iterator returns the entries in ascending key order. The sets's spliterator is <em><a
     ...
     * iteration over the set is in progress (except through the iterator's own {@code remove} operation, or through the {@code setValue}
     * operation on a map entry returned by the iterator) the results of the iteration are undefined. The set supports element removal,
     * which removes the corresponding mapping from the map, via the {@code Iterator.remove}, {@code Set.remove}, {@code removeAll}, {@code
     * retainAll} and {@code clear} operations. It does not support the {@code add} or {@code addAll} operations.
     */
     ...
     */
    abstract class PrivateEntryIterator implements Iterator<Map.Entry<K, V>> {
