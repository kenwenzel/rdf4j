...
final class LmdbUtil {
	...
	static int mergeOld(long cursor, int elements, MDBVal keyVal, MDBVal dataVal,
	...
	static int merge(long cursor, int elements, MDBVal keyVal, MDBVal dataVal,
		ByteBuffer newValueBuf, ByteBuffer target) throws IOException {
		return mergeNew(cursor, elements, keyVal, dataVal, newValueBuf, target);
	}

	static int mergeNew(long cursor, int elements, MDBVal keyVal, MDBVal dataVal,
		ByteBuffer newValueBuf, ByteBuffer target) throws IOException {
		...
		if (rc == MDB_SUCCESS) {
			...
			if (compareRegion(newValueBuf, 0, buffer, 0, Math.min(newValueBuf.remaining(), buffer.remaining())) == 0) {
				// The new value is equal to the first duplicate value >= newValueBuf. The tuple already exists in this chunk.
				return MDB_KEYEXIST;
			}
			// The new value is greater than the first duplicate value >= newValueBuf. Step back to the previous duplicate value.
		...
		if (addValueToSecondChunk || existing.hasNext()) {
			...
			while (existing.hasNext()) {
				try {
					...
					int length = newValueBuf.limit();
					byte[] data = new byte[length];
					newValueBuf.get(0, data, 0, length);

					System.out.println("Error encoding tuple: " + Arrays.toString(data));
