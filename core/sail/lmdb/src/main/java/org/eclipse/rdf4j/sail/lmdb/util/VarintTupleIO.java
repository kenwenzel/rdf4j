...
public final class VarintTupleIO {
	...
	public static class Encoder {
		...
		public void append(ByteBuffer tuple) {
			...
			for (int i = 0; i < elements; i++) {
				...
				final int currentLength = Varint.firstToLength(out.get(currentValuePosition));
				if (out.get(rawPosition) == 0) {
					// Zero is a reuse marker, not the encoded value itself.
					rawPosition++;
				} else {
					rawPosition += currentLength;
				}

				final int otherLength = Varint.firstToLength(tuple.get(otherValuePosition));
				int result = compareRegion(out, currentValuePosition, tuple, otherValuePosition,
					Math.min(currentLength, otherLength));

				if (result == 0) {
					out.put((byte) 0);
				} else {
					reusePositions[i] = out.position();
					out.put(out.position(), tuple, otherValuePosition, otherLength);
					out.position(out.position() + otherLength);
				}
		...
		public void appendNextTuple(VarintTupleIO input) {
			...
			for (int i = 0; i < elements; i++) {
				...
				boolean wasEncoded = input.nextPosition - buffer.position() > 1 && Varint.readUnsigned(buffer, buffer.position()) > 0;
				if (DEBUG) {
					System.out.println("appendNextTuple: element " + i + " at " + buffer.position() + " wasEncoded=" + wasEncoded);
					System.out.println("comparing " + Varint.readUnsigned(buffer, otherValuePosition) + " with " + Varint.readUnsigned(out, currentValuePosition) + " (" + currentValuePosition + ", " + out.position() + "), reusePositions=" + java.util.Arrays.toString(reusePositions));
				}

				int result = compareRegion(out, currentValuePosition, buffer, otherValuePosition,
					Math.min(currentLength, otherLength));

				if (DEBUG) {
					System.out.println("compared " + Varint.readUnsigned(buffer, otherValuePosition) + " with " + Varint.readUnsigned(out, currentValuePosition) + " (" + currentValuePosition + ", " + out.position() + "), reusePositions=" + java.util.Arrays.toString(reusePositions));
					var a = 1;
				}
				...
				if (result == 0) {
					if (out.position() == out.limit()) {
						...
						buffer.get(0, data, 0, length);

						System.out.println("input: " + java.util.Arrays.toString(data));

						length = out.limit();
						data = new byte[length];
						out.get(0, data, 0, length);

						System.out.println("output: " + java.util.Arrays.toString(data));
					}
					// System.out.println("reuse: slot " + i + " at " + out.position() + " from " + reusePositions[i] + " for " + Varint.readUnsigned(buffer, otherValuePosition));
					out.put((byte) 0);
				} else {
					var pos = out.position();
					reusePositions[i] = pos;
					if (DEBUG) {
						System.out.println("writing: slot " + i + " at " + pos + " for " + Varint.readUnsigned(buffer, otherValuePosition));
					}
					out.put(pos, buffer, otherValuePosition, otherLength);
					out.position(pos + otherLength);
					if (DEBUG) {
						System.out.println("wrote: slot " + i + " at " + pos + " for " + Varint.readUnsigned(out, pos));
					}
				}
			}
			lastTuplePosition = rawPosition;
	 ...
	 * <p>
	 * Reuse markers in this cursor's tuple are resolved through {@code reusePositions}.
	 * This method does not change the cursor state or either buffer's position.
	 * </p>
	 *
	 * @param otherTuple buffer positioned at the first element of the tuple to compare
	 * @return a negative value, zero, or a positive value if this tuple is respectively
	 *         less than, equal to, or greater than {@code otherTuple}
	 */
	public int compareTuple(ByteBuffer otherTuple) {
		...
		for (int i = 0; i < elements; i++) {
			...
			if (buffer.get(rawPosition) == 0) {
				// Zero is a reuse marker, not the encoded value itself.
				currentValuePosition = reusePositions[i];
				currentLength = Varint.firstToLength(buffer.get(currentValuePosition));
				rawPosition++;
			} else {
			...
			final int otherLength = Varint.firstToLength(otherTuple.get(otherValuePosition));
			int result = compareRegion(buffer, currentValuePosition, otherTuple, otherValuePosition,
				Math.min(currentLength, otherLength));
